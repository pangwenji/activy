...
async function createServer(isProd = process.env.NODE_ENV === 'production') {
    ...
    app.use(routes.map(e => (e + '/:data?')),async (req, res, next) => {
        ...
        try {
            ...
            let cookie = version ? ["version="+encodeURIComponent(JSON.stringify([version]))] : [];
            const headers = {
            ...
            let html = template.replace('<!--app-ssr-->',appHtml);
