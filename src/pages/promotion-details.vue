<template>
	<page :on-pullup="onReachBottom" :on-pulldown="onPullDownRefresh" :show-header="isPc ? false :true">
		<div class="page-k page-pading">
			...
			<table-card>
				...
				<template #row>
					...
					<table-row v-for="v in list" class="table-row">
						...
						<table-cell class="cell">
							<template #cell>
								{{ t('pageAgent.fyT',{num:v.compute_rate}) }}
							</template>
						</table-cell>
						<table-cell class="cell">
							<template #cell>
			...
			<ui-pagination class="pagination" v-if="isPc && list.length" v-model="pages" :pages="lastPage" @change="getCommissionLog()"></ui-pagination>
			</div>
		</div>
